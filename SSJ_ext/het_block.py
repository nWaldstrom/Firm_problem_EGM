...
class HetBlock:
    ...
    def __init__(self, back_step_fun, exogenous, policy, backward):
        ...
        self.inputs_p =  set(self.backward)
        
        # input checking
        if self.exogenous not in self.all_inputs:
    ...
    def ss(self, backward_tol=1E-10, backward_maxit=11000, forward_tol=1E-10, forward_maxit=300_000, accelerated_it = True, **kwargs):
        ...
        hetinputs = self.make_inputs(kwargs)
        Pi = hetinputs[self.exogenous].copy()
...
# 
    ...
    def td(self, ss, monotonic=False, returnindividual=False, **kwargs):
        ...
        D = ss['D']
        Pi_path = np.empty((T,) + ss[self.exogenous].shape)
        ...
        for t in reversed(range(T)):
            ...
            hetinputs = self.make_inputs(backdict) # get Markov matrix for hetinput 
            Pi_path[t] = hetinputs[self.exogenous].T.copy()
            
        D_path = np.empty((T,) + D.shape)
        
        
        # update dist at time 0 
        sspol_i = {}
        sspol_pi = {}
        for pol in self.policy:
        ...
        D_path[0, ...] =  self.forward_step(D, Pi_path[0, ...], sspol_i, sspol_pi)
        ...
        for t in range(T-1):
            ...
            # step forward
            D_path[t+1, ...]= self.forward_step(D_path[t, ...], Pi_path[t+1, ...], sspol_i, sspol_pi)
    ...
    def jac(self, ss, T, shock_list, output_list=None, h=1E-4, save=False, use_saved=False):
        ...
        (ssin_dict, Pi_ss, ssout_list, ss_for_hetinput, 
        ...
        curlyYs, curlyDs = {}, {}
        for i in shock_list:
            curlyYs[i], curlyDs[i], Pi_shock, curlyD0 = self.backward_iteration_fakenews(i, output_list, ssin_dict, ssout_list,
                                                ss['D'], Pi_ss.T.copy(), sspol_i, sspol_pi, sspol_space, T, ss, h,
                                                ss_for_hetinput)
        ...
        for o in output_list:
            curlyPs[o], curlyP_T[o] = self.forward_iteration_fakenews(ss[o], Pi_ss, sspol_i, sspol_pi, T-1, curlyD0)
            dD0_eff[o] = self.build_dD0(curlyPs[o], curlyP_T[o], curlyD0, T)
        ...
        for o in output_list:
            for i in shock_list:
                ...
                J[o.upper()][i] = HetBlock.J_from_F(F[o.upper()][i], dD0_eff[o])
    ...
    def build_dD0(self, curlyPs, curlyP_T, curlyD0, T):
        
        #print(curlyPs['a'].shape, curlyD0.shape)
        
        hej = np.zeros((T, T))   
        # for t in range(T):
        #     #h = np.vdot(curlyPs['a'][t,...].T , curlyD0)
        #     #print(h)
        #     if t < T-1:
        #         hej[t,0] = np.vdot(curlyPs[t,...] , curlyD0)
        #     else:
        #         hej[t,0] = np.vdot(curlyP_T , curlyD0)
        return hej
        ...
        if use_saved and 'curlyYs' in self.saved:
            ...
            for i in shock_list:
                curlyYs[i], curlyDs[i] = self.backward_iteration_fakenews(i, output_list, 
                                                    ssin_dict, ssout_list, ss['D'], Pi_ss.T.copy(), sspol_i, 
                                                    sspol_pi, sspol_space, T, ss, h, ss_for_hetinput)

            # step 2: compute prediction vectors curlyP (forward iteration) for each outcome o
            # here go to (T-1) + (Tpost-1) rather than (T-1)
            curlyPs = {}            
            for o in output_list:
                curlyPs[o], _ = self.forward_iteration_fakenews(ss[o], Pi_ss, sspol_i, sspol_pi, T-1+Tpost-1)
    ...
    def howard_improvement_algorithm(consumption_function, transition_matrix, ssin):
        period_utility = u(consumption_function, ssin['eis'])
    
        v = spsolve(eye(params.Nm)-ssin['beta']*transition_matrix, period_utility)
    ...
    def policy_ss(self, ssin, Pi, tol=1E-10, maxit=3000, accelerated_it = True, noisy = False, update_pol=None, nIt = 3, conv_var = 'policy', disp_itt = 100, accelerated_it_end=0,  **kwargs):
        ...
        # step = {}
        # old_guess = {}
        # old_guess.update({k: ssin[k+ '_p'] for k in self.backward})  
        # hs = {k: ssin[k+ '_p'] for k in self.backward}
        # v_past_period = {k: ssin[k+ '_p'] for k in self.backward}
    ...
    def backward_step_fakenews(self, din_dict, output_list, ssin_dict, ssout_list, 
                               Dss, Pi_T_ss, Pi_T_shock, sspol_i, sspol_pi, sspol_space, h=1E-4):
        ...
        curlyD = self.forward_step_shock(Dss, Pi_T_ss, sspol_i, sspol_pi, pol_pi_shock, Pi_T_shock)
    ...
    def backward_iteration_fakenews(self, input_shocked, output_list, ssin_dict, ssout_list, Dss, Pi_T_ss,
                                    sspol_i, sspol_pi, sspol_space, T, ss, h=1E-4, ss_for_hetinput=None):
        ...
        if 'Pi_p' in din_dict:
            Pi_shock_T = din_dict['Pi_p'].T.copy() 
        else:
            Pi_shock_T = np.zeros_like(Pi_T_ss)
        ...
        curlyV, curlyD, curlyY = self.backward_step_fakenews(din_dict, output_list, ssin_dict, ssout_list, 
                                                             Dss, Pi_T_ss, Pi_shock_T, sspol_i, sspol_pi, sspol_space, h)
        ...
        # CurlyD at time 0 
        curlyD0 = self.forward_step(Dss, Pi_T_ss+Pi_shock_T, sspol_i, sspol_pi) - Dss
        #curlyD0 = self.forward_step(Dss, Pi_T_ss, sspol_i, sspol_pi) - Dss
        

        # fill in current effect of shock
        curlyDs[0, ...] = curlyD #+ curlyD0
        for k in curlyY.keys():
            curlyYs[k][0] = curlyY[k] +  np.vdot(curlyD0,  ss[k])

        

        # fill in anticipation effects
        Pi_shock = np.empty((T,) + Pi_T_ss.shape)
        for t in range(1, T):
            if t == 1 and 'Pi_p' in din_dict:
                Pi_shock_T =  np.zeros_like(Pi_T_ss) # din_dict['Pi_p'].T.copy()  
            else:
                Pi_shock_T= np.zeros_like(Pi_T_ss)
        ...
        for t in range(0, T):
            if t == 0 and 'Pi_p' in din_dict:
                Pi_shock[t] = din_dict['Pi_p'].copy()  
            else:
                Pi_shock[t] = np.zeros_like(Pi_T_ss)
            
        return curlyYs, curlyDs, Pi_shock, curlyD0

    def forward_iteration_fakenews(self, o_ss, Pi, pol_i_ss, pol_pi_ss, T, Pi_shock):
        ...
        for t in range(1, T):
            ...
            if t == (T-1):
                curlyP_T =  utils.demean(self.forward_step_transpose(curlyPs[t, ...], Pi, pol_i_ss, pol_pi_ss))
        return curlyPs, curlyP_T
    ...
    def forward_step_shock(self, Dss, Pi_T_ss, pol_i_ss, pol_pi_ss, pol_pi_shock, Pi_T_shock):
        """Forward_step linearized with respect to pol_pi"""
        if len(self.policy) == 1:
            p, = self.policy
            return utils.forward_step_shock_1d(Dss, Pi_T_ss, pol_i_ss[p], pol_pi_shock[p], pol_pi_ss[p], Pi_T_shock)
        elif len(self.policy) == 2:
            p1, p2 = self.policy
            return utils.forward_step_shock_2d(Dss, Pi_T_ss, pol_i_ss[p1], pol_i_ss[p2],
