...
@njit
def forward_step_shock_1d(Dss, Pi_T_ss, x_i_ss, x_pi_shock, x_pi_ss, Pi_shock):
    ...
    return  Pi_T_ss @ Dshock + (Pi_shock @ Dss)
...
def numerical_diff(func, ssinputs_dict, shock_dict, h=1E-4, y_ss_list=None):
    ...
    #print( shock_dict['pi_e'])
    #print(ssinputs_dict['pi_e'])
...
def broyden_solver_jac(f,x0,jac,tol=1e-8,max_iter=100,backtrack_fac=0.5,max_backtrack=30,do_print=False):
...
""" quadrature Functions for quadrature (mostly Gauss-Hermite) from ConSav package """
